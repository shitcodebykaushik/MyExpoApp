...
const HomeScreen = ({ navigation }) => {
  ...
  const [teachersOnLeave, setTeachersOnLeave] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [attendanceRes, cgpaRes, examRes, studyRes, timetableRes, annRes, teachersRes] = await Promise.all([
          ...
          axios.get(`${API_BASE_URL}/student/teachers-on-leave`, { headers }),
        ]);
        ...
        setTeachersOnLeave(teachersRes.data.teachers_on_leave);
  ...
  const services = [
    { id: "1", name: "Attendance", icon: "calendar-check", value: attendance ? `${attendance.percentage}%` : "--" },
    { id: "2", name: "CGPA", icon: "chart-line", value: cgpa ? `${cgpa.cgpa}` : "--" },
    { id: "3", name: "Exams", icon: "file-document", value: examCount },
    { id: "4", name: "Study Material", icon: "book-open", value: `${studyMaterials.length}` },
    { id: "5", name: "Timetable", icon: "clock-outline", value: timetable ? "View" : "--" },
    { id: "6", name: "Announcements", icon: "bell-outline", value: announcementCount },
  ];
...
const styles = StyleSheet.create({
  ...
  logoutButton: { backgroundColor: "red", padding: 10, marginTop: 20, alignItems: "center", borderRadius: 5 },
